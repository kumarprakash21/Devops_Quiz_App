...
router.post("/", async (request, response, next) => {
  try {
    ...
    if (!section || !Number.isInteger(correct) || !Number.isInteger(total) || !Number.isInteger(durationSeconds) || total < 1 || correct < 0 || correct > total || durationSeconds < 0) return response.status(400).json({ error: "Invalid assessment result." });
    const percentage = Math.round((correct / total) * 100);
    request.user.attempts.unshift({ section, correct, total, percentage, passed: percentage >= 70, durationSeconds });
